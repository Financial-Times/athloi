...
async function version (targetPackages = [], tag, allPackages = []) {
	...
	if (semver.valid(number)) {
		...
		throw new Error(`The given tag "${tag}" is not a valid version number`);
	}
	...
	return targetPackages.map(pkg => {
		const apply = () => {
			const newManifest = updateVersions(
				pkg.manifest,
				packagesToUpdate,
				number,
				latestVersions,
...
exports.register = program => {
	program
		.command('version <tag>')
		.description(
			'Updates the release number for public packages and their cross-dependencies and writes the data back to package.json',
		)
